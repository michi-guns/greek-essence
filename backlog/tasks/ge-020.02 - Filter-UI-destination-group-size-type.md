---
id: GE-020.02
title: 'Filter UI (destination, group size, type)'
status: To Do
assignee: []
created_date: '2026-09-22 07:21'
labels:
  - 'lane:product'
milestone: m-3
dependencies: []
parent_task_id: GE-020
type: feature
ordinal: 92000
---

## Description

<!-- SECTION:DESCRIPTION:BEGIN -->
Planning ref: T-03.12 (docs/v1/04-ROADMAP.md)
est: 90/165/330 min (min/avg/max)
Phase P3.3 - Product lane

Τεταρτο use client component. Φιλτραρισμα client-side πανω στο ηδη φορτωμενο payload, χωρις server round-trip.
<!-- SECTION:DESCRIPTION:END -->

## Definition of Done
<!-- DOD:BEGIN -->
- [ ] #1 Acceptance criteria met; final summary records what was checked and what remains
- [ ] #2 typecheck, lint and unit tests pass
- [ ] #3 User-visible change: live walk completed per A-007 with evidence in .local/evidence/ - or state why not applicable
- [ ] #4 Anything odd noticed is filed as a task, not dropped
- [ ] #5 Task branch merged into its phase branch
<!-- DOD:END -->
