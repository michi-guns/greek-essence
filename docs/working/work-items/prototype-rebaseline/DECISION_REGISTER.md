...
| ID | Decision | Status | Current evidence | Needed before approval |
| --- | --- | --- | --- | --- |
| ADR-001 | Product capability and module boundaries | Candidate | Marketing, Catalogue, Consultation, Booking, and CRM contexts are reported; prior `discovery` proposal is withdrawn | Confirm source authority, release scope, context interactions, and code-boundary mapping |
| ADR-002 | CMS product and editorial model | Candidate | Sanity is reported as public-content owner | Confirm roles, workflow, localization, preview authorization, media, revisions, environments, recovery, and cost |
| ADR-003 | Supabase responsibility boundary | Candidate | Supabase is reported as transactional-data owner | Decide Postgres, Auth, Storage, Realtime, Edge Functions, RLS, backups, regions, and environment separation |
| ADR-004 | Drizzle responsibility boundary | Candidate | Drizzle is reported for schema access and reproducible migrations | Decide schema/migration ownership, server query boundary, pooling, transactions, generated types, and RLS interaction |
...
| ADR-008 | Runtime and deployment topology | Candidate | Public production-facing Phase 1 is confirmed; Vercel, Sanity, Supabase, CI, preview, and production environments are reported | Confirm regions, connections, migration runner, jobs/webhooks, secrets, monitoring, backup, recovery, and rollback ownership |
...
| ADR-016 | Notification delivery and recovery | Open | Agency and customer notifications are required | Choose providers, templates, retry/idempotency, diagnostics, and manual recovery |
